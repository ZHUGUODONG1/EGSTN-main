...
class DataLoader_multi_modal(object):
    ...
    def shuffle(self):
        permutation = np.random.permutation(self.size)
        x1, y1, x2, y2, x3, y3 = self.x1[permutation], self.y1[permutation], self.x2[permutation], self.y2[permutation], \
                                 self.x3[permutation], self.y3[permutation]
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
...
def load_CS(pkl_filename):
    CS = load_pickle(pkl_filename)
    return CS
...
def load_dataset_multi_modal(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    data['train_multi_modal_loader'] = DataLoader_multi_modal(data['x_train'], data['y_train'], data['x_train_text'],
                                                              data['y_train_text'], data['x_train_index'],
                                                              data['y_train_index'], batch_size)
    data['val_multi_modal_loader'] = DataLoader_multi_modal(data['x_val'], data['y_val'], data['x_val_text'],
                                                            data['y_val_text'], data['x_val_index'],
                                                            data['y_val_index'], valid_batch_size)
    data['test_multi_modal_loader'] = DataLoader_multi_modal(data['x_test'], data['y_test'], data['x_test_text'],
                                                             data['y_test_text'], data['x_test_index'],
                                                             data['y_test_index'], test_batch_size)

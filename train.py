...
parser.add_argument('--model', type=str, default='EGSTN', help='model type')
...
parser.add_argument('--S_in_dim', type=int, default=3, help='space-specific factor dim')
parser.add_argument('--D_in_dim', type=int, default=4, help=' time sensitive factor dim')
...
class trainer():
    def __init__(self, in_dim, seq_length, num_nodes, nhid, l, dropout, lrate, wdecay, device, supports,CS, decay,S_in_dim,D_in_dim):
        self.model = EGSTN(device, num_nodes, l, dropout, supports=supports,CS=CS ,length=seq_length,
                             S_in_dim=S_in_dim,D_in_dim=D_in_dim,
...
def main():
    ...
    print(args)
    if args.model == 'EGSTN':
        engine = trainer(args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.l, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, CS, args.decay,args.S_in_dim,args.D_in_dim
                         )

...
class DelayEstimators(nn.Module):
    ...
    def forward(self, Trend, CD):
        ...
        for i in range(C):
            K = self.conv_layers_K[i](CD_split[i]).reshape(B, self.out_channels, -1)  
...
class SAGPool(nn.Module):
    def __init__(self, in_channels, length, pool_ratio=0.5):
        ...
        self.gcn = GCN1(in_channels*length,1)  
        self.sigmoid = nn.Sigmoid()
...
class MultiScaleGCN(nn.Module):
    def __init__(self, in_channels, out_channels, length, num_nodes, pool_ratio1=0.5,pool_ratio2=0.1,eb_dim=10):
        ...
        self.pool1 = SAGPool(in_channels,length, pool_ratio=pool_ratio1)  
        self.pool2 = SAGPool(in_channels,length, pool_ratio=pool_ratio2)  
        self.self_attention = SelfAttention(3*out_channels,out_channels)
        self.E1 = nn.Parameter(torch.randn(num_nodes, eb_dim))  
        self.E2 = nn.Parameter(torch.randn(eb_dim, num_nodes))  
        self.E11 = nn.Parameter(torch.randn(int(num_nodes*pool_ratio1), eb_dim))  
        self.E21 = nn.Parameter(torch.randn(eb_dim, int(num_nodes*pool_ratio1))) 
        self.E12 = nn.Parameter(torch.randn(int(num_nodes*pool_ratio2), eb_dim))  
        self.E22 = nn.Parameter(torch.randn(eb_dim, int(num_nodes*pool_ratio2)))  
